...
! default values
  ...
  READ (9, '(A1)') buf !"-----dd---------- number of blocks in a cluster along the X-direction")')
  READ (9, '(5x,i2)') cluster_N_blocks_x
  READ (9, '(A1)') buf !"-----dd---------- number of blocks in a cluster along the Y-direction")')
  READ (9, '(5x,i2)') cluster_N_blocks_y
...
!        READ (9, '(A1)') buf !"---ddddd.d------- value of the potential at this point [V]
 i_given_F_double_period_sys=40
 j_given_F_double_period_sys=40
 given_F_double_period_sys=0
...
!  END IF
  ...
  open(10, file='v1.yaml', status='old')
  read(10, *) dummy, dummy
  read(10, *) dummy, dummy
  read(10, *) dummy, dummy
  read(10, *) dummy, dummy
  ...
  read(10, *) dummy, dummy
  read(10, *) dummy, dummy
  read(10, *) dummy, t_sim_ns
  read(10, *) dummy, N_subcycles
  read(10, *) dummy, dT_cluster_load_balance
  read(10, *) dummy, dT_global_load_balance
  read(10, *) dummy, dT_save_checkpoint
  read(10, *) dummy, use_checkpoint
  read(10, *) dummy, T_cntr_to_continue
  PRINT '(2x,"Test ",i8," : Is it upup")', T_cntr_to_continue
  close(10)
...
! the IF branch above should work for periodic systems with segmented electrodes
  ...
  INQUIRE (FILE = 'init_Bx_input.dat', EXIST = exists)
  CALL MPI_BARRIER(MPI_COMM_WORLD, ierr)


  IF (Rank_of_process.EQ.0) THEN
     PRINT '(2x,"Process ",i3," : init_Bx_input.dat is found. Reading the data file...")', Rank_of_process
  END IF

 OPEN (9, FILE = 'init_Bx_input.dat')


  ALLOCATE(Bxx(1:1151,1:2841), STAT = ALLOC_ERR)
!ALLOCATE(Byy(1:1280,1:1280), STAT = ALLOC_ERR)

  DO i=1,2841
    READ (9,'(f11.6)') (Bxx(j,i),j=1,1151)
  END DO

  CLOSE (9, STATUS = 'KEEP')
 
 INQUIRE (FILE = 'init_By_input.dat', EXIST = exists)
 CALL MPI_BARRIER(MPI_COMM_WORLD, ierr)


  IF (Rank_of_process.EQ.0) THEN
     PRINT '(2x,"Process ",i3," : init_By_input.dat is found. Reading the data file...")', Rank_of_process
  END IF

  OPEN (9, FILE = 'init_By_input.dat')


!ALLOCATE(Bxx(1:1280,1:1280), STAT = ALLOC_ERR)
  ALLOCATE(Byy(1:1151,1:2841), STAT = ALLOC_ERR)

  DO i=1,2841
   READ (9,'(f11.6)') (Byy(j,i),j=1,1151)
 END DO
 ...
 DO i = 1, 1151
   DO j = 1, 2841
     Byy(i,j) = Byy(i, j)*1.0d-4/B_scale_T/1
     Bxx(i,j) = Bxx(i, j)*1.0d-4/B_scale_T/1
  END DO
END DO
...
!  END IF
  ...
  open(10, file='v1.yaml', status='old')
  read(10, *) dummy, dummy
  read(10, *) dummy, init_Te_eV
  read(10, *) dummy, init_Ne_m3
  read(10, *) dummy, N_spec
  ...
  DO s = 1, N_spec
  read(10, *) dummy, M_i_amu(s)
  read(10, *) dummy, Qs(s)
  read(10, *) dummy, init_Ti_eV(s)
  read(10, *) dummy, init_NiNe(s)
  init_NiNe(s) = 0.01_8 * init_NiNe(s)
  END DO
  
  IF (N_spec.EQ.1) THEN
     init_NiNe(1) = 1.0_8
  read(10, *) dummy, init_random_seed
  END IF

!  PRINT '(2x,"Test ",e10.3," : Is it right")', init_Ne_m3
  close(10)
     ...
     DO j = c_indx_y_min, c_indx_y_max
        DO i = c_indx_x_min, c_indx_x_max
           WRITE (10, '(2x,i5,2x,i5,2x,f12.9,2x,f12.9,2x,e14.7,2x,e14.7)') &
                ...
                & i * delta_x_m, &
                & j * delta_x_m, &
                & B_scale_T * 1.0d4 * Bxx(i, j), &
                & B_scale_T * 1.0d4 * Byy(i, j)
        END DO
        WRITE (10, '(" ")')
     END DO
...
!        END IF
        ...
        myBx_T = B_scale_T * Bxx(int(electron(k)%X), int(electron(k)%Y))
        myBy_T = B_scale_T * Byy(int(electron(k)%X), int(electron(k)%Y))
        myBz_T = B_scale_T * 0
