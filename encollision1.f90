...
!  END IF
  ...
  read(10, *) dummy, dummy
  read(10, *) dummy, dummy
  read(10, *) dummy, N_neutral_spec
  PRINT '(2x,"Test ",i2," : Is it leftleftleft")', N_neutral_spec 
  ...
  read(10, *) dummy, neutral(n)%name
  read(10, *) dummy, neutral(n)%N_m3
  PRINT '(2x,"Test ",e10.3," : Is it right")', neutral(n)%N_m3
  read(10, *) dummy, neutral(n)%T_K
  END DO
  close(10)
...
! find maximal fraction of colliding particles for the neutral density neutral(n)%N_m3
    ...
    ELSE 
            ...
            norm_factor = 1.0_8
    END IF
   PRINT '(2x,"Numax ",f10.2," : null collsion")', temp
