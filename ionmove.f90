...
!        E_Y = acc_EY(i, j) * ax_i * ay_j + acc_EY(i+1, j) * ax_ip1 * ay_j + acc_EY(i, j+1) * ax_i * ay_jp1 + acc_EY(i+1, j+1) * ax_ip1 * ay_jp1        
        ...
        ELSE 
           IF (ions_sense_Ez) THEN
                E_Z = 0 ! Aug-3-2017 found a bug here, N_subcycles was missing
           ELSE
                E_Z = 0.0_8
           END IF
        END IF
...
! calculate magnetic field factors   !##### MODIFY FOR IONS ########
           ...
           alfa_x = QM2 * Bxx(int(Xarg), int(Yarg)) * delta_t_factor * N_subcycles
           alfa_y = QM2 * Byy(int(Xarg), int(Yarg)) * delta_t_factor * N_subcycles
           alfa_z = QM2 * 0 * delta_t_factor * N_subcycles
